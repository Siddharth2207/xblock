...
import {
    LOCK_TOKEN, LOCK_TOKEN_HOLDER, VAULT_ID, OrderV2, LibTrancheRefillOrders
...
contract XBlockModelling is XBlockStratUtil {
    ...
    function test_trancheModelling() public {
        ...
        vm.writeLine(file, string.concat("Tranche space", ",", "Amount", ",", "Price"));

        for (uint256 i = 0; i < 200; i++) {
            uint256 trancheSpace = uint256(1e17 * i);
            vm.mockCall(
                address(STORE),
                abi.encodeWithSelector(IInterpreterStoreV1.get.selector, namespace, trancheSpaceKey),
                abi.encode(trancheSpace)
            );
            vm.mockCall(
                address(STORE),
                abi.encodeWithSelector(IInterpreterStoreV1.get.selector, namespace, lastUpdateTimeKey),
            ...
            string memory line = string.concat(uint2str(trancheSpace), ",", uint2str(stack[1]), ",", uint2str(stack[0]));
    ...
    function test_lastUpdateTime() public {
        ...
        vm.writeLine(file, string.concat("Time (seconds)", ",", "Amount", ",", "Price"));
        ...
        for (uint256 i = 0; i < (48 * 8); i++) {
            ...
            string memory line = string.concat(uint2str(time), ",", uint2str(stack[1]), ",", uint2str(stack[0]));
    ...
    function test_trancheAmounts() public {
        ...
        vm.writeLine(file, string.concat("Tranche", ",", "Tranche Amount", ",", "Tranche Price"));
        ...
        for (uint256 i = 0; i < 200; i++) {
            ...
            string memory line = string.concat(
                uint2str(trancheSpace / 1e18), ",", uint2str(stack[1]), ",", uint2str(stack[0] * 1e18 / stack[2])
            );
